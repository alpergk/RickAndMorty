...
class CharacterListVC: RMDataLoadingVC {
    ...
    private func updateSnapshot(for character: RMCharacter) {
        var snapshot = dataSource.snapshot()
        snapshot.reconfigureItems([character])
        DispatchQueue.main.async {
            self.dataSource.apply(snapshot, animatingDifferences: true)
        }
    }
...
extension CharacterListVC: UICollectionViewDelegate {
    ...
    func removeCharacterFromFavorites(character: RMCharacter) {
        let favorite = RMFavoriteCharacter(name: character.name, avatarURL: character.image, id: character.id)
        PersistenceManager.updateWith(favorite: favorite, actionType: .remove) { [weak self] error in
            ...
            guard let error else {
                DispatchQueue.main.async {
                    self.presentRMAlert(title: "Success!", message: "Deleted from favorites \(character.name)", buttonTitle: "Ok")
                }
                return
            }
            DispatchQueue.main.async {
    ...
    func didTapFavoriteButton(for character: RMCharacter) {
        ...
        Task  {
            ...
            do {
                ...
                if PersistenceManager.isFavorite(character: character) {
                    removeCharacterFromFavorites(character: character)
